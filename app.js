...
function renderHome() {
  ...
  return `<div class="screen home-page"><div class="page-scroll"><main class="page-content with-nav">${topHome()}
    ...
    <div class="home-modules">
      ...
      <section class="home-module home-planner"><div class="section-head home-module-head"><h2 class="section-title">智能营销策划</h2>${hotelSelect()}</div><div class="campaign-suite"><button class="campaign-card" data-action="marketing-open"><span class="campaign-copy"><span class="campaign-kicker">营销活动 · 本期主推</span><h3>暑期亲子入住推广</h3><p>AI结合酒店情况，形成图片内容和视频内容。</p><span class="campaign-meta"><span>8月中下旬</span><span class="campaign-cta">开始策划 ${icon("chevron")}</span></span></span><span class="campaign-visual" aria-hidden="true"><span class="campaign-sheet campaign-sheet-back"></span><span class="campaign-sheet campaign-sheet-front"></span><span class="campaign-orbit"></span><span class="campaign-light"></span></span></button><div class="idea-row"><button class="idea-link" data-action="marketing-topic" data-topic="早餐体验推广">早餐体验推广</button><button class="idea-link" data-action="marketing-topic" data-topic="周末度假">周末度假</button><button class="idea-link" data-action="marketing-topic" data-topic="会议接待">会议接待</button></div></div></section>
...
function renderMarketingPlan() {
  ...
  const adjust = state.marketingAdjust;
  const recommendationReason = adjustedPlan ? "结合酒店近期情况，把入住和周边亲子体验连起来，让家庭客人更容易理解这一趟轻度假安排。" : "暑期最后两周仍有短途亲子出行需求。现在适合把亲子房、早餐和周边体验串联起来，突出住得舒服、带孩子方便、顺便有得玩。";
  ...
  const content = `<div class="marketing-plan-content"><section class="marketing-section"><h2 class="marketing-section-title">活动信息</h2><div class="marketing-topic-card"><div class="marketing-topic-topline"><span class="marketing-topic-tag">AI 推荐营销活动</span></div><h2 class="marketing-topic-title">${esc(topic)}</h2><div class="marketing-hotel-line"><span class="marketing-hotel-mark">${esc(hotelLogo)}</span><span>本次酒店</span><strong>${esc(hotelName)}</strong></div><span class="marketing-hotel-address">${esc(hotelAddress)}</span></div></section><section class="marketing-section"><h2 class="marketing-section-title">推荐理由</h2><div class="marketing-surface-card"><div class="marketing-reason-row"><span>推荐原因</span><strong>${recommendationReason}</strong></div><div class="marketing-metric-grid"><div><span>建议时间</span><strong>8月中下旬</strong></div><div><span>目标客群</span><strong>周末亲子家庭</strong></div></div></div></section><section class="marketing-section"><h2 class="marketing-section-title">策划详情</h2><div class="marketing-surface-card marketing-detail-list"><div><span>宣传目标</span><strong>让家庭客人产生“现在就去住一晚”的入住兴趣</strong></div><div><span>宣传重点</span><p>${focusItems}</p></div><div><span>表达方向</span><strong>${expressionDirection}</strong></div><div><span>素材建议</span><strong>${materialSuggestion}</strong></div></div></section><section class="marketing-section"><h2 class="marketing-section-title">内容清单</h2><div class="marketing-deliverables"><div><span class="marketing-deliverable-icon">${icon("image")}</span><span><strong>图片内容</strong><small>宣传图＋图片文案</small></span><em>待生成</em></div><div><span class="marketing-deliverable-icon">${icon("video")}</span><span><strong>视频内容</strong><small>15秒视频＋视频文案</small></span><em>待生成</em></div></div></section><section class="marketing-section marketing-adjust-section"><button class="marketing-adjust-row" data-action="marketing-adjust-open" aria-haspopup="dialog" aria-controls="marketing-adjust-sheet" aria-expanded="${adjust.open ? "true" : "false"}"><span><strong>${adjust.pending ? "查看调整结果" : "补充一点情况"}</strong><small>${adjust.pending ? "有一版调整后的策划等待确认" : "补充酒店近期情况，我会基于当前活动再调整一版"}</small></span>${icon("chevron")}</button></section></div>`;
...
function renderMarketingCreate() {
  ...
  const typeSelector = `<section class="section"><div class="section-head"><h2 class="section-title">内容类型</h2><span class="section-note">选择一种先生成</span></div><div class="segmented"><button class="segment ${image ? "active" : ""}" data-action="marketing-type" data-type="image">图片＋图片文案</button><button class="segment ${!image ? "active" : ""}" data-action="marketing-type" data-type="video">视频＋视频文案</button></div></section>`;
...
function marketingPackageDone(completed, kind) {
  const media = kind === "video" ? "15秒视频" : "宣传图";
  const copy = kind === "video" ? "视频文案" : "图片文案";
  return completed.includes(media) && (completed.includes(copy) || completed.includes("通用发布文案"));
}

function marketingPackageLabel(kind) {
  return kind === "video" ? "视频＋视频文案" : "图片＋图片文案";
}
...
function generationMeta(gen) {
  ...
  const marketingCopyLabel = gen.kind === "video" ? "视频文案" : "图片文案";
  const currentCombo = isMarketing ? `${mediaLabel}＋${marketingCopyLabel}` : mediaLabel;
  const copyDone = completed.includes(marketingCopyLabel) || completed.includes("通用发布文案");
  const pending = isMarketing ? [mediaLabel, marketingCopyLabel].filter((item) => item === mediaLabel ? !completed.includes(item) : !copyDone) : [mediaLabel];
  ...
  const steps = isMarketing ? ["读取营点策划", `生成${marketingCopyLabel}`, `生成${mediaLabel}`, "整理内容组合"] : isTemplate ? ["读取模板结构", "替换主图与酒店信息", "生成宣传图", "整理作品信息"] : ["理解创作想法", "组合素材与酒店信息", `生成${mediaLabel}画面`, "整理作品信息"];
...
function generationActions() {
  return `<div class="generation-exit-actions"><button class="primary-btn" data-action="generation-view-works">查看作品</button><button class="secondary-btn" data-action="generation-home">返回首页</button></div>`;
}
...
function renderResult() {
  ...
  const marketingCopy = video ? "这个夏天，带孩子住得舒服，也顺便有得玩。亲子房、早餐和周边体验，一次安排好。" : "暑期最后两周，带孩子住得舒服，也顺便有得玩。亲子房、早餐和周边体验，一次安排好。";
  const marketingCopyLabel = video ? "视频文案" : "图片文案";
  const marketingNext = video ? "宣传图" : "15秒视频";
  const marketingNextKind = video ? "image" : "video";
  const marketingDone = marketingPackageLabel(kind);
  const marketingProgressLabel = marketingPackageDone(marketingCompleted, kind) ? "已完成" : "本次内容";
  const marketingAllDone = marketingPackageDone(marketingCompleted, "image") && marketingPackageDone(marketingCompleted, "video");
  const marketingFollowup = marketingAllDone ? "" : `<section class="section"><div class="editorial-card"><div class="editorial-card-body"><p class="eyebrow">营销活动后续</p><h2 class="section-title">继续完成${marketingNext}内容</h2><p class="section-note">接着生成${marketingPackageLabel(marketingNextKind)}。</p><button class="text-action" data-action="marketing-continue" data-kind="${marketingNextKind}">继续生成${marketingNext}内容 ${icon("chevron")}</button></div></div></section>`;
  const marketingContent = marketing ? `<section class="section marketing-result-section"><div class="section-head"><div><p class="eyebrow">营销活动</p><h2 class="section-title">暑期尾声亲子入住推广</h2></div><div class="marketing-result-head-actions"><span class="status-pill">${marketingAllDone ? "已完成" : "进行中"}</span><button class="text-action" data-nav="marketingActivity">查看活动详情 ${icon("chevron")}</button></div></div><div class="copy-block marketing-result-copy"><div class="copy-block-head"><strong>${marketingCopyLabel}</strong><button class="text-action" data-action="copy">复制${marketingCopyLabel}</button></div><p>${marketingCopy}</p></div><p class="marketing-result-progress">${marketingProgressLabel}：${esc(marketingDone)}</p></section>${marketingFollowup}` : "";
...
function renderMarketingActivityV2() {
  const completed = marketingCompletedItems();
  const imageDone = marketingPackageDone(completed, "image");
  const videoDone = marketingPackageDone(completed, "video");
  const allDone = imageDone && videoDone;
  const nextKind = imageDone ? "video" : "image";
  const nextLabel = imageDone ? "视频" : "图片";
  const hotel = currentHotel();
  ...
  const contentRows = `${imageDone ? completedRow("image", "图片内容", "宣传图＋图片文案", "image") : pendingRow("图片内容", "宣传图＋图片文案", "image")}${videoDone ? completedRow("video", "视频内容", "15秒视频＋视频文案", "video") : pendingRow("视频内容", "15秒视频＋视频文案", "video")}`;
  const content = `<section class="section"><div class="activity-hero"><div class="activity-hero-head"><p class="eyebrow">营销活动</p><span class="status-pill">${allDone ? "已完成" : "进行中"}</span></div><h2 class="section-title">暑期亲子入住推广</h2><div class="activity-meta"><span>${esc(hotel?.name || "云栖酒店·杭州")}</span><span>今天 10:24 更新</span></div></div></section><section class="section"><div class="section-head"><h2 class="section-title">推荐理由</h2><span class="section-note">已认可策划</span></div><div class="surface-card activity-plan-card"><div class="summary-list"><div class="summary-item"><span>推荐原因</span><strong>暑期最后两周仍有短途亲子出行需求。现在适合把亲子房、早餐和周边体验串联起来，突出住得舒服、带孩子方便、顺便有得玩。</strong></div><div class="summary-item"><span>建议时间</span><strong>8月中下旬</strong></div><div class="summary-item"><span>目标客群</span><strong>周末亲子家庭</strong></div></div></div></section><section class="section"><div class="section-head"><h2 class="section-title">策划详情</h2></div><div class="surface-card activity-plan-card"><div class="summary-list"><div class="summary-item"><span>宣传目标</span><strong>让家庭客人产生“现在就去住一晚”的入住兴趣</strong></div><div class="summary-item"><span>宣传重点</span><strong>亲子房、早餐和酒店周边亲子去处</strong></div><div class="summary-item"><span>表达方向</span><strong>明亮、轻松、有具体入住画面，让人感到这趟周末安排很省心</strong></div><div class="summary-item"><span>素材建议</span><strong>客房、早餐、家庭入住场景和酒店周边亲子去处照片</strong></div></div></div></section><section class="section"><div class="section-head"><div><h2 class="section-title">内容清单</h2><p class="section-note">图片内容和视频内容来自同一份策划</p></div></div><div class="activity-content-list">${contentRows}</div>${allDone ? `<p class="activity-complete-note">图片内容和视频内容均已生成，本次营销活动已完成。</p>` : `<button class="primary-btn activity-next-action" data-action="marketing-continue" data-kind="${nextKind}">继续生成${nextLabel}内容</button>`}</section>`;
  return pageShell({ title: "营销活动详情", content });
}
...
function renderWorks() {
  ...
  const marketingCompleted = marketingCompletedItems();
  const marketingImageDone = marketingPackageDone(marketingCompleted, "image") || (!marketingCompleted.length && !activeMarketingGeneration);
  const marketingVideoDone = marketingPackageDone(marketingCompleted, "video");
...
function addWorkFromGeneration() {
  ...
  if (marketing) {
    const completed = [...new Set([...(gen.completed || []), gen.kind === "video" ? "视频文案" : "图片文案", gen.kind === "video" ? "15秒视频" : "宣传图"])];
