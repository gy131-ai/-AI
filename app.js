...
function renderHome() {
  ...
  return `<div class="screen home-page"><div class="page-scroll"><main class="page-content with-nav">${topHome()}
    ...
    <div class="home-modules">
      ...
      <section class="home-module home-planner"><div class="section-head home-module-head"><h2 class="section-title">智能营销策划</h2>${hotelSelect()}</div><div class="campaign-suite"><button class="campaign-card" data-action="marketing-open"><span class="campaign-copy"><span class="campaign-kicker">营销活动 · 本期主推</span><h3>暑期亲子入住推广</h3><p>AI结合酒店情况，形成通用文案、宣传图和15秒视频。</p><span class="campaign-meta"><span>8月中下旬</span><span class="campaign-cta">开始策划 ${icon("chevron")}</span></span></span><span class="campaign-visual" aria-hidden="true"><span class="campaign-sheet campaign-sheet-back"></span><span class="campaign-sheet campaign-sheet-front"></span><span class="campaign-orbit"></span><span class="campaign-light"></span></span></button><div class="idea-row"><button class="idea-link" data-action="marketing-topic" data-topic="早餐体验推广">早餐体验推广</button><button class="idea-link" data-action="marketing-topic" data-topic="周末度假">周末度假</button><button class="idea-link" data-action="marketing-topic" data-topic="会议接待">会议接待</button></div></div></section>
...
function renderResult() {
  ...
  const marketingAllDone = marketingCompleted.includes("宣传图") && marketingCompleted.includes("15秒视频");
  const marketingFollowup = marketingAllDone ? "" : `<section class="section"><div class="editorial-card"><div class="editorial-card-body"><p class="eyebrow">营销活动后续</p><h2 class="section-title">继续完成${marketingNext}</h2><p class="section-note">共享通用文案会自动复用，接着生成${marketingNext}。</p><button class="text-action" data-action="marketing-continue" data-kind="${marketingNextKind}">继续生成${marketingNext} ${icon("chevron")}</button></div></div></section>`;
  const marketingContent = marketing ? `<section class="section marketing-result-section"><div class="section-head"><div><p class="eyebrow">营销活动</p><h2 class="section-title">暑期尾声亲子入住推广</h2></div><div class="marketing-result-head-actions"><span class="status-pill">${marketingAllDone ? "已完成" : "进行中"}</span><button class="text-action" data-nav="marketingActivity">查看活动详情 ${icon("chevron")}</button></div></div><div class="copy-block marketing-result-copy"><div class="copy-block-head"><strong>通用发布文案</strong><button class="text-action" data-action="copy">复制文案</button></div><p>${marketingCopy}</p></div><p class="marketing-result-progress">已完成：${esc(marketingDone)}</p></section>${marketingFollowup}` : "";
...
function renderMarketingActivityV2() {
  ...
  const imageDone = completed.includes("宣传图");
  const videoDone = completed.includes("15秒视频");
  const allDone = copyDone && imageDone && videoDone;
  const nextKind = imageDone ? "video" : "image";
  const nextLabel = imageDone ? "视频" : "图片";
  const hotel = currentHotel();
